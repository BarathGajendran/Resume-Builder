...
const EMPTY_RESUME: ResumeData = {
  ...
  summary: '', experience: [], internships: [], education: [], projects: [],
  certifications: [], languages: [], volunteer: [], skills: [],
};
...
function TagInput({ tags, onChange, placeholder = 'Type and press Enter…' }: { tags: string[]; onChange: (t: string[]) => void; placeholder?: string }) {
  ...
  return (
    <div className="border border-[#e0dbd2] rounded-sm p-2.5 focus-within:border-[#c5a059] transition-all cursor-text" onClick={() => ref.current?.focus()}>
      ...
      <p className="text-[9px] text-[#d0ccc5]">Enter or comma to add · Backspace to remove last</p>
    </div>
  );
}
  ...
  return (
    <div className="border border-[#ebe7e0] rounded-sm overflow-hidden">
      <div
        ...
        tabIndex={0}
        aria-expanded={open}
        className="flex items-center justify-between px-3.5 py-2.5 bg-[#faf9f6] cursor-pointer select-none"
        onClick={() => setOpen(o => !o)}
...
/* ─── Resume Preview (also used for print) ───────────────── */
export function ResumePreview({ data, printRef }: { data: ResumeData; printRef?: React.RefObject<HTMLDivElement | null> }) {
  const p = data.personalInfo;
  return (
    <div ref={printRef} id="resume-print" className="bg-white font-serif-body text-[#222]">
      {/* Header */}
      ...
      <div className="px-12 py-9 space-y-8 text-[12.5px] leading-[1.75]">
        {data.summary && (
          <section><PH title="Profile"/><p className="text-[#444] text-justify">{data.summary}</p></section>
        )}
        ...
        {data.experience.length > 0 && (
          <section><PH title="Experience"/>{data.experience.map(it => <PExp key={it.id} item={it}/>)}</section>
        )}
        {data.internships.length > 0 && (
          <section><PH title="Internships"/>{data.internships.map(it => <PExp key={it.id} item={it}/>)}</section>
        )}
            ...
            <div className="space-y-3.5">
              {data.certifications.map(it => (
                <div key={it.id} className="flex items-start gap-4">
                  <div className="w-1 shrink-0 mt-1 rounded-full bg-[#e0dbd2] self-stretch"/>
                  <div>
...
export function ResumeBuilder() {
  ...
  const navSections = [
    ...
    { id: 'experience', label: 'Experience' }, { id: 'internships', label: 'Internships' },
    ...
    { id: 'volunteer', label: 'Volunteer' }, { id: 'skills', label: 'Skills' },
  ];
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#f4f1ec] font-sans no-print-layout">
      {/* ══ LEFT: Editor ══════════════════════════════════════ */}
      <div className="editor-panel w-[420px] shrink-0 flex flex-col bg-white border-r border-[#e8e4de] shadow-[2px_0_16px_rgba(0,0,0,0.04)]">

        {/* Top bar */}
        <div className="px-5 py-4 border-b border-[#f0ede8] shrink-0 flex items-end justify-between">
          <div>
        ...
        <div className="px-5 py-3.5 border-t border-[#f0ede8] shrink-0">
          <button type="button" onClick={handlePrint}
            className="w-full flex items-center justify-center gap-2 bg-[#1a1a1a] text-white py-2.5 text-[11px] font-semibold tracking-[0.12em] uppercase hover:bg-[#333] transition-colors rounded-sm">
      ...
      {/* ══ RIGHT: Live Preview ════════════════════════════════ */}
      <div className="preview-panel flex-1 overflow-y-auto cscroll bg-[#e8e4dc] py-10 px-8">
        <div className="max-w-[700px] mx-auto shadow-[0_8px_40px_rgba(0,0,0,0.12)]">
